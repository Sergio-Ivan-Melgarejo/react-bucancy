...
const FavoritesItem = ({favorite,handleDeleteSong}) => {
    const navigate = useNavigate()

    const handleDelete = () =>{
        handleDeleteSong(favorite.id)
    }

    const handlePlay = () =>{
        navigate(`/song/${favorite.id}`)
    }

  return (
    <button className='favorite__container'>
        <img className='favorite__logo' src={favorite.src} alt={favorite.band} />
